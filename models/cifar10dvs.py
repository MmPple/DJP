...
from .submodules.sparse import ConvBlock, TemporalBatchNorm
...
class VGGSNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = ConvBlock(conv3x3(2, 64), nn.BatchNorm2d(64), create_mslif(),
                               sparse_neurons=False, sparse_weights=True)
        self.conv2 = ConvBlock(conv3x3(64, 128), TemporalBatchNorm(128, 10), create_mslif(),
                               sparse_neurons=True, sparse_weights=True)
        self.pool1 = nn.AvgPool2d(2, 2)
        self.conv3 = ConvBlock(conv3x3(128, 256), TemporalBatchNorm(256, 10), create_mslif(),
                               sparse_neurons=True, sparse_weights=True)
        self.conv4 = ConvBlock(conv3x3(256, 256), TemporalBatchNorm(256, 10), create_mslif(),
                               sparse_neurons=True, sparse_weights=True)
        self.pool2 = nn.AvgPool2d(2, 2)
        self.conv5 = ConvBlock(conv3x3(256, 512), TemporalBatchNorm(512, 10), create_mslif(),
                               sparse_neurons=True, sparse_weights=True)
        self.conv6 = ConvBlock(conv3x3(512, 512), TemporalBatchNorm(512, 10), create_mslif(),
                               sparse_neurons=True, sparse_weights=True)
        self.pool3 = nn.AvgPool2d(2, 2)
        self.conv7 = ConvBlock(conv3x3(512, 512), TemporalBatchNorm(512, 10), create_mslif(),
                               sparse_neurons=True, sparse_weights=True)
        self.conv8 = ConvBlock(conv3x3(512, 512), TemporalBatchNorm(512, 10), create_mslif(),
                               sparse_neurons=True, sparse_weights=True)
        self.pool4 = nn.AvgPool2d(2, 2)
        ...
        self.init_weight()
        self.spike_list = []
        self.sparse_spikes = False

    def init_weight(self):
        for m in self.modules():
            if isinstance(m, (nn.Conv2d, nn.Linear)):
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
            elif isinstance(m, (nn.BatchNorm2d, TemporalBatchNorm)):
                if(m.weight != None):
    ...
    def forward(self, x: torch.Tensor):
        if(self.sparse_spikes):
            self.spike_list.clear()
        x = x.transpose(0, 1)
        #### [N, T, C, H, W] -> [T, N, C, H, W]
        x = self.conv1(x)
        if(self.sparse_spikes):
            self.spike_list.append(x)
        x = self.conv2(x)
