...
def update_lmbda(model: nn.Module):
    cnt = 0
    for m in model.modules():
        frontC = None
        if isinstance (m, ConvBlock):
            if(m.conv.weight.size(-1) == 1): continue
            if(frontC):
                m.weight_mask.lmbda = ConvBlock.spike_list[cnt].detach().sum(dim=[0,1,3,4]).view(1,-1,1,1)
                cnt += 1
            if(frontC and frontC.neuron_mask):
                frontC.neuron_mask.lmbda = m.weight_mask.mask().sum(dim=[0,2,3]).view(-1,1,1)
                frontC.tlmbda = frontC.neuron_mask.lmbda.view(1, -1)
            frontC = m
        ...
        for name, m in net.named_modules():
            ...
            if isinstance(m, ConvBlock):
                ...
                if m.sparse_neurons:
                    ...
                    mask = (m.neuron_mask.mask() != 0).float().squeeze(0)
                else:
                    mask = None
