...
class TemperatureScheduler:
    ...
    def linear_factor(self):
        return 1.0 * (self.current_step - self.T0) / (self.Tmax - self.T0)
    
    def exp_factor(self):
        return self._temp() / self.final_temp

    def p_factor(self):
        return (1.0 * (self.current_step - self.T0) / (self.Tmax - self.T0)) ** 2
    
    def cos_factor(self):
        return 1 - math.cos(math.pi * (self.current_step - self.T0) / (2 * (self.Tmax - self.T0)))
